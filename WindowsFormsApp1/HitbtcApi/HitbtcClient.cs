...
namespace WindowsFormsApp1
{
    ...
    class HitbtcSocketClient : IDisposable
    {
        ...
        public HitbtcSocketClient(string key, string secret)
        {
            _apiKey = key;
            _secretKey = secret;
        ...
        public HitbtcSocketClient() : this("", "") { }
        ...
        public List<HitbalanceData> GetAccountbalance()
        {
            RestClient client1 = new RestClient(ApiUrl);
            var request1 = new RestRequest("/api/2/account/balance", Method.GET);
            client1.Authenticator = new HttpBasicAuthenticator(_apiKey, _secretKey);
            request1.RequestFormat = DataFormat.Json;
            var Content = client1.Execute(request1).Content;
            return JsonConvert.DeserializeObject<List<HitbalanceData>>(Content);
        }
